// ============================================
// Feature: Terminales — CRUD (Vanilla TS)
// ============================================
...
import * as terminalesService from './terminales.service';
import { terminalesHeaders } from './datatable_config/terminales.headers';
import { toTerminalesGridRows } from './datatable_config/terminales.body';
import { getTerminalesFields } from './form_config/terminales.fields';
import { buildDeleteBody } from './form_config/terminales.delete';
import type { Terminales, TerminalesCreateDTO } from './terminales.types';
...
export class TerminalesFeature {
  // State
  private _terminales: Terminales[] = [];
  private _saving = false;
  private _selectedTerminales: Terminales | null = null;
  ...
  mount(container: HTMLElement): void {
    ...
    pageTitle.textContent = 'Terminales';
    pageTitle.style.margin = '0';

    const btnNew = document.createElement('button');
    btnNew.textContent = '+ Nuevo Terminales';
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._terminales = filterExcluded(await terminalesService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private _preSelectFKValues(form: HTMLElement, data: Terminales): void {
  ...
  private _refreshGrid(): void {
    this._gridie.setBody(
      toTerminalesGridRows(this._terminales, {
  ...
  private _openCreate(): void {
    this._selectedTerminales = null;
    this._modalCreate.setBody(this._buildForm(null, this._modalCreate));
    this._modalCreate.open();
  }

  private _openEdit(item: Terminales): void {
    this._selectedTerminales = item;
  ...
  private _openDelete(item: Terminales): void {
    this._selectedTerminales = item;
  ...
  private _buildForm(initialData: Terminales | null, modal: ModalXInstance): HTMLElement {
    ...
    const form = FormX({
      ...
      onSubmit: async (result) => {
        ...
        try {
          const data: TerminalesCreateDTO = {
          ...
          if (isEdit) {
            ...
            toastx.success('Terminales creado correctamente');
          }
      ...
      children: [
        ...getTerminalesFields(initialData, {
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedTerminales || this._saving) return;
      this._saving = true;
...
export function Terminales(container: HTMLElement): TerminalesFeature {
  const feature = new TerminalesFeature();
