...
export function buildDeleteBody(item: Ordenes): HTMLElement {
  const wrapper = document.createElement('div');

  const question = document.createElement('p');
  question.style.marginBottom = '16px';
  question.textContent = '¿Estás seguro que deseas eliminar el siguiente ordenes?';
  ...
  secondary.textContent = String(item.terminal_id ?? '');
