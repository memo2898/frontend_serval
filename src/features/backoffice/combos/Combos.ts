// ============================================
// Feature: Combos — CRUD (Vanilla TS)
// ============================================
...
import * as combosService from './combos.service';
import { combosHeaders } from './datatable_config/combos.headers';
import { toCombosGridRows } from './datatable_config/combos.body';
import { getCombosFields } from './form_config/combos.fields';
import { buildDeleteBody } from './form_config/combos.delete';
import type { Combos, CombosCreateDTO } from './combos.types';
...
export class CombosFeature {
  // State
  private _combos: Combos[] = [];
  private _saving = false;
  private _selectedCombos: Combos | null = null;
  ...
  mount(container: HTMLElement): void {
    ...
    pageTitle.textContent = 'Combos';
    pageTitle.style.margin = '0';

    const btnNew = document.createElement('button');
    btnNew.textContent = '+ Nuevo Combos';
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._combos = filterExcluded(await combosService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private _preSelectFKValues(form: HTMLElement, data: Combos): void {
  ...
  private _openDelete(item: Combos): void {
    this._selectedCombos = item;
  ...
  private _buildForm(initialData: Combos | null, modal: ModalXInstance): HTMLElement {
      ...
      onSubmit: async (result) => {
        ...
        try {
          ...
          if (isEdit) {
            ...
            toastx.success('Combos creado correctamente');
          }
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedCombos || this._saving) return;
      this._saving = true;
...
export function Combos(container: HTMLElement): CombosFeature {
  const feature = new CombosFeature();
