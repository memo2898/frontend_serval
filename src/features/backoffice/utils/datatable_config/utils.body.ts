import type { Utils, UtilsGridRow } from '../utils.types';

export interface UtilsHandlers {
  onEdit: (utils: Utils) => void;
  onDelete: (utils: Utils) => void;
}
...
export const toUtilsGridRow = (utils: Utils, handlers: UtilsHandlers): UtilsGridRow => ({
  id: utils.id,
  nombre: utils.nombre,
  descripcion: utils.descripcion,
  actions: [
    {
      content: `
        ${actionButtonStyles}
    ...
    },
    {
      ...
      `,
      event: 'click',
      funct: () => handlers.onDelete(utils),
    },
  ],
});

export const toUtilsGridRows = (utils: Utils[], handlers: UtilsHandlers): UtilsGridRow[] =>
  utils.map((utils) => toUtilsGridRow(utils, handlers));
