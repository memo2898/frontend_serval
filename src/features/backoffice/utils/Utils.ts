...
import { utilsHeaders } from './datatable_config/utils.headers';
import { toUtilsGridRows } from './datatable_config/utils.body';
import { getUtilsFields } from './form_config/utils.fields';
import { buildDeleteBody } from './form_config/utils.delete';
import type { Utils, UtilsCreateDTO } from './utils.types';
...
export class UtilsFeature {
  // State
  private _utils: Utils[] = [];
  private _saving = false;
  private _selectedUtils: Utils | null = null;
  ...
  mount(container: HTMLElement): void {
    ...
    pageTitle.textContent = 'Utils';
    pageTitle.style.margin = '0';

    const btnNew = document.createElement('button');
    btnNew.textContent = '+ Nuevo Utils';
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._utils = filterExcluded(await utilsService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private _buildForm(initialData: Utils | null, modal: ModalXInstance): HTMLElement {
      ...
      onSubmit: async (result) => {
        ...
        try {
          ...
          if (isEdit) {
            ...
            toastx.success('Utils creado correctamente');
          }
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedUtils || this._saving) return;
      this._saving = true;
...
export function Utils(container: HTMLElement): UtilsFeature {
  const feature = new UtilsFeature();
