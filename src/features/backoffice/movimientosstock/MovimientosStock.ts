// ============================================
// Feature: MovimientosStocks — CRUD (Vanilla TS)
// ============================================
...
import * as movimientosStockService from './movimientosstock.service';
import { movimientosStockHeaders } from './datatable_config/movimientosstock.headers';
import { toMovimientosStockGridRows } from './datatable_config/movimientosstock.body';
import { getMovimientosStockFields } from './form_config/movimientosstock.fields';
import { buildDeleteBody } from './form_config/movimientosstock.delete';
import type { MovimientosStock, MovimientosStockCreateDTO } from './movimientosstock.types';
...
export class MovimientosStockFeature {
  // State
  private _movimientosStocks: MovimientosStock[] = [];
  private _saving = false;
  private _selectedMovimientosStock: MovimientosStock | null = null;
  ...
  mount(container: HTMLElement): void {
    ...
    pageTitle.textContent = 'MovimientosStocks';
    pageTitle.style.margin = '0';

    const btnNew = document.createElement('button');
    btnNew.textContent = '+ Nuevo MovimientosStock';
    ...
    this._gridie = new Gridie({
      id: 'movimientosstock-table',
      identityField: 'id',
      headers: movimientosStockHeaders,
      body: [],
      enableSort: true,
      enableFilter: true,
      language: 'es',
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._movimientosStocks = filterExcluded(await movimientosStockService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private async _fetchRelatedOptions(): Promise<void> {
    try {
      const [articulosRaw, sucursalesRaw, ordenesRaw] = await Promise.all([
  ...
  private _preSelectFKValues(form: HTMLElement, data: MovimientosStock): void {
  ...
  private _refreshGrid(): void {
    this._gridie.setBody(
      toMovimientosStockGridRows(this._movimientosStocks, {
  ...
  private _openCreate(): void {
    this._selectedMovimientosStock = null;
    this._modalCreate.setBody(this._buildForm(null, this._modalCreate));
    this._modalCreate.open();
  }

  private _openEdit(item: MovimientosStock): void {
    this._selectedMovimientosStock = item;
  ...
  private _openDelete(item: MovimientosStock): void {
    this._selectedMovimientosStock = item;
  ...
  private _buildForm(initialData: MovimientosStock | null, modal: ModalXInstance): HTMLElement {
    ...
    const form = FormX({
      ...
      onSubmit: async (result) => {
        ...
        try {
          const data: MovimientosStockCreateDTO = {
          ...
          if (isEdit) {
            ...
            toastx.success('MovimientosStock creado correctamente');
          }
      ...
      children: [
        ...getMovimientosStockFields(initialData, {
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedMovimientosStock || this._saving) return;
      this._saving = true;
...
export function MovimientosStock(container: HTMLElement): MovimientosStockFeature {
  const feature = new MovimientosStockFeature();
