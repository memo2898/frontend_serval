...
export function buildDeleteBody(item: MovimientosStock): HTMLElement {
  const wrapper = document.createElement('div');

  const question = document.createElement('p');
  question.style.marginBottom = '16px';
  question.textContent = '¿Estás seguro que deseas eliminar el siguiente movimientosstock?';
  ...
  secondary.textContent = String(item.sucursal_id ?? '');
