import type { Stock, StockGridRow } from '../stock.types';

export interface StockHandlers {
...
export const toStockGridRow = (stock: Stock, handlers: StockHandlers): StockGridRow => ({
  id: stock.id,
  ...
  actions: [
    ...
    },
    {
      ...
      `,
      event: 'click',
      funct: () => handlers.onDelete(stock),
    },
  ],
});

export const toStockGridRows = (stocks: Stock[], handlers: StockHandlers): StockGridRow[] =>
  stocks.map((stock) => toStockGridRow(stock, handlers));
