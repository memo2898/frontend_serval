import type { Mail, MailGridRow } from '../mail.types';
...
export const toMailGridRow = (mail: Mail, handlers: MailHandlers): MailGridRow => ({
  id: mail.id,
  to: mail.to,
  ...
  actions: [
    ...
    },
    {
      ...
      `,
      event: 'click',
      funct: () => handlers.onDelete(mail),
    },
  ],
});

export const toMailGridRows = (mails: Mail[], handlers: MailHandlers): MailGridRow[] =>
  mails.map((mail) => toMailGridRow(mail, handlers));
