// ============================================
// Feature: RolPermisos — CRUD (Vanilla TS)
// ============================================
...
import * as rolPermisoService from './rolpermiso.service';
import { rolPermisoHeaders } from './datatable_config/rolpermiso.headers';
import { toRolPermisoGridRows } from './datatable_config/rolpermiso.body';
import { getRolPermisoFields } from './form_config/rolpermiso.fields';
import { buildDeleteBody } from './form_config/rolpermiso.delete';
import type { RolPermiso, RolPermisoCreateDTO } from './rolpermiso.types';
import * as rolesService from '@/features/backoffice/roles/roles.service';
import * as permisosService from '@/features/backoffice/permisos/permisos.service';
...
export class RolPermisoFeature {
  // State
  private _rolPermisos: RolPermiso[] = [];
  private _saving = false;
  private _selectedRolPermiso: RolPermiso | null = null;
  private _rolesOptions: Array<{ value: number; label: string }> = [];
  private _permisosOptions: Array<{ value: number; label: string }> = [];
  ...
  mount(container: HTMLElement): void {
    ...
    pageTitle.textContent = 'RolPermisos';
    pageTitle.style.margin = '0';

    const btnNew = document.createElement('button');
    btnNew.textContent = '+ Nuevo RolPermiso';
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._rolPermisos = filterExcluded(await rolPermisoService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private async _fetchRelatedOptions(): Promise<void> {
    try {
      const [rolesRaw, permisosRaw] = await Promise.all([
        rolesService.getAll(),
      ...
      this._permisosOptions = filterExcluded(permisosRaw).map((item: any) => ({
        value: item.id,
  ...
  private _preSelectFKValues(form: HTMLElement, data: RolPermiso): void {
  ...
  private _refreshGrid(): void {
    this._gridie.setBody(
      toRolPermisoGridRows(this._rolPermisos, {
  ...
  private _openCreate(): void {
    this._selectedRolPermiso = null;
    this._modalCreate.setBody(this._buildForm(null, this._modalCreate));
    this._modalCreate.open();
  }

  private _openEdit(item: RolPermiso): void {
    this._selectedRolPermiso = item;
  ...
  private _openDelete(item: RolPermiso): void {
    this._selectedRolPermiso = item;
  ...
  private _buildForm(initialData: RolPermiso | null, modal: ModalXInstance): HTMLElement {
    ...
    const form = FormX({
      ...
      onSubmit: async (result) => {
        ...
        try {
          const data: RolPermisoCreateDTO = {
            rol_id: result.body['rol_id'] as number,
            permiso_id: result.body['permiso_id'] as number,
          };
          ...
          if (isEdit) {
            ...
            toastx.success('RolPermiso creado correctamente');
          }
      ...
      children: [
        ...getRolPermisoFields(initialData, {
          rolesOptions: this._rolesOptions,
          permisosOptions: this._permisosOptions,
        }),
        errorMsg,
        actions,
      ],
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedRolPermiso || this._saving) return;
      this._saving = true;
...
export function RolPermiso(container: HTMLElement): RolPermisoFeature {
  const feature = new RolPermisoFeature();
