import type { RolPermiso, RolPermisoGridRow } from '../rolpermiso.types';

export interface RolPermisoHandlers {
...
export const toRolPermisoGridRow = (rolPermiso: RolPermiso, handlers: RolPermisoHandlers): RolPermisoGridRow => ({
  id: rolPermiso.id,
  ...
  actions: [
    {
      ...
      `,
      event: 'click',
      funct: () => handlers.onEdit(rolPermiso),
    },
    {
      content: `
      ...
      `,
      event: 'click',
      funct: () => handlers.onDelete(rolPermiso),
    },
  ],
});

export const toRolPermisoGridRows = (rolPermisos: RolPermiso[], handlers: RolPermisoHandlers): RolPermisoGridRow[] =>
  rolPermisos.map((rolPermiso) => toRolPermisoGridRow(rolPermiso, handlers));
