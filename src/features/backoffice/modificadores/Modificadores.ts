// ============================================
// Feature: Modificadores — CRUD (Vanilla TS)
// ============================================
...
import * as modificadoresService from './modificadores.service';
import { modificadoresHeaders } from './datatable_config/modificadores.headers';
import { toModificadoresGridRows } from './datatable_config/modificadores.body';
import { getModificadoresFields } from './form_config/modificadores.fields';
import { buildDeleteBody } from './form_config/modificadores.delete';
import type { Modificadores, ModificadoresCreateDTO } from './modificadores.types';
...
export class ModificadoresFeature {
  // State
  private _modificadores: Modificadores[] = [];
  private _saving = false;
  private _selectedModificadores: Modificadores | null = null;
  ...
  mount(container: HTMLElement): void {
    ...
    pageTitle.textContent = 'Modificadores';
    pageTitle.style.margin = '0';

    const btnNew = document.createElement('button');
    btnNew.textContent = '+ Nuevo Modificadores';
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._modificadores = filterExcluded(await modificadoresService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private _refreshGrid(): void {
    this._gridie.setBody(
      toModificadoresGridRows(this._modificadores, {
  ...
  private _buildForm(initialData: Modificadores | null, modal: ModalXInstance): HTMLElement {
    ...
    const form = FormX({
      ...
      onSubmit: async (result) => {
        ...
        try {
          ...
          if (isEdit) {
            ...
            toastx.success('Modificadores creado correctamente');
          }
      ...
      children: [
        ...getModificadoresFields(initialData),
        errorMsg,
        actions,
      ],
    });
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedModificadores || this._saving) return;
      this._saving = true;
...
export function Modificadores(container: HTMLElement): ModificadoresFeature {
  const feature = new ModificadoresFeature();
