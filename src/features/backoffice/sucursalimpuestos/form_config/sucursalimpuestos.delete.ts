...
export function buildDeleteBody(item: SucursalImpuestos): HTMLElement {
  const wrapper = document.createElement('div');

  const question = document.createElement('p');
  question.style.marginBottom = '16px';
  question.textContent = '¿Estás seguro que deseas eliminar el siguiente sucursalimpuestos?';
  ...
  secondary.textContent = String(item.impuesto_id ?? '');
