// ============================================
// Feature: Impuestos — CRUD (Vanilla TS)
// ============================================
...
import * as impuestosService from './impuestos.service';
import { impuestosHeaders } from './datatable_config/impuestos.headers';
import { toImpuestosGridRows } from './datatable_config/impuestos.body';
import { getImpuestosFields } from './form_config/impuestos.fields';
import { buildDeleteBody } from './form_config/impuestos.delete';
import type { Impuestos, ImpuestosCreateDTO } from './impuestos.types';
import * as paisesService from '@/features/backoffice/paises/paises.service';
...
export class ImpuestosFeature {
  // State
  private _impuestos: Impuestos[] = [];
  private _saving = false;
  private _selectedImpuestos: Impuestos | null = null;
  private _paisesOptions: Array<{ value: number; label: string }> = [];
  ...
  mount(container: HTMLElement): void {
    ...
    pageTitle.textContent = 'Impuestos';
    pageTitle.style.margin = '0';

    const btnNew = document.createElement('button');
    btnNew.textContent = '+ Nuevo Impuestos';
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._impuestos = filterExcluded(await impuestosService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private async _fetchRelatedOptions(): Promise<void> {
    try {
      const [paisesRaw] = await Promise.all([
        paisesService.getAll(),
      ]);
      this._paisesOptions = filterExcluded(paisesRaw).map((item: any) => ({
        value: item.id,
  ...
  private _preSelectFKValues(form: HTMLElement, data: Impuestos): void {
  ...
  private _refreshGrid(): void {
    this._gridie.setBody(
      toImpuestosGridRows(this._impuestos, {
  ...
  private _openCreate(): void {
    this._selectedImpuestos = null;
    this._modalCreate.setBody(this._buildForm(null, this._modalCreate));
    this._modalCreate.open();
  }

  private _openEdit(item: Impuestos): void {
    this._selectedImpuestos = item;
  ...
  private _openDelete(item: Impuestos): void {
    this._selectedImpuestos = item;
  ...
  private _buildForm(initialData: Impuestos | null, modal: ModalXInstance): HTMLElement {
    ...
    const form = FormX({
      ...
      onSubmit: async (result) => {
        ...
        try {
          const data: ImpuestosCreateDTO = {
            pais_id: result.body['pais_id'] as number,
            nombre: result.body['nombre'] as string,
            porcentaje: result.body['porcentaje'] as number,
            tipo: result.body['tipo'] as string,
            tipo_aplicacion: result.body['tipo_aplicacion'] as string,
          };
          ...
          if (isEdit) {
            ...
            toastx.success('Impuestos creado correctamente');
          }
      ...
      children: [
        ...getImpuestosFields(initialData, {
          paisesOptions: this._paisesOptions,
        }),
        errorMsg,
        actions,
      ],
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedImpuestos || this._saving) return;
      this._saving = true;
...
export function Impuestos(container: HTMLElement): ImpuestosFeature {
  const feature = new ImpuestosFeature();
