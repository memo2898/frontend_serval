import type { Uploads, UploadsGridRow } from '../uploads.types';

export interface UploadsHandlers {
  onEdit: (uploads: Uploads) => void;
  onDelete: (uploads: Uploads) => void;
}
...
export const toUploadsGridRow = (uploads: Uploads, handlers: UploadsHandlers): UploadsGridRow => ({
  ...
  actions: [
    ...
    },
    {
      ...
      `,
      event: 'click',
      funct: () => handlers.onDelete(uploads),
    },
  ],
});

export const toUploadsGridRows = (uploads: Uploads[], handlers: UploadsHandlers): UploadsGridRow[] =>
  uploads.map((uploads) => toUploadsGridRow(uploads, handlers));
