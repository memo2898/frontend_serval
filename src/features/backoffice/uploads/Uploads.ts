// ============================================
// Feature: Uploads — CRUD (Vanilla TS)
// ============================================
...
import { getUploadsFields } from './form_config/uploads.fields';
import { buildDeleteBody } from './form_config/uploads.delete';
import type { Uploads, UploadsCreateDTO } from './uploads.types';
...
export class UploadsFeature {
  // State
  private _uploads: Uploads[] = [];
  private _saving = false;
  private _selectedUploads: Uploads | null = null;
  ...
  mount(container: HTMLElement): void {
    ...
    btnNew.textContent = '+ Nuevo Uploads';
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._uploads = filterExcluded(await uploadsService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private _refreshGrid(): void {
    this._gridie.setBody(
      toUploadsGridRows(this._uploads, {
  ...
  private _buildForm(initialData: Uploads | null, modal: ModalXInstance): HTMLElement {
    ...
    const form = FormX({
      ...
      onSubmit: async (result) => {
        ...
        try {
          const data: UploadsCreateDTO = {
            0: result.body['0'] as string,
          };
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedUploads || this._saving) return;
      this._saving = true;
...
export function Uploads(container: HTMLElement): UploadsFeature {
  const feature = new UploadsFeature();
