import type { Zonas, ZonasGridRow } from '../zonas.types';

export interface ZonasHandlers {
  onEdit: (zonas: Zonas) => void;
  onDelete: (zonas: Zonas) => void;
}
...
export const toZonasGridRow = (zonas: Zonas, handlers: ZonasHandlers): ZonasGridRow => ({
  ...
  actions: [
    ...
    },
    {
      ...
      `,
      event: 'click',
      funct: () => handlers.onDelete(zonas),
    },
  ],
});

export const toZonasGridRows = (zonas: Zonas[], handlers: ZonasHandlers): ZonasGridRow[] =>
  zonas.map((zonas) => toZonasGridRow(zonas, handlers));
