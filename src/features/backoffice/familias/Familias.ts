// ============================================
// Feature: Familias — CRUD (Vanilla TS)
// ============================================
...
import * as familiasService from './familias.service';
import { familiasHeaders } from './datatable_config/familias.headers';
import { toFamiliasGridRows } from './datatable_config/familias.body';
import { getFamiliasFields } from './form_config/familias.fields';
import { buildDeleteBody } from './form_config/familias.delete';
import type { Familias, FamiliasCreateDTO } from './familias.types';
...
export class FamiliasFeature {
  // State
  private _familias: Familias[] = [];
  private _saving = false;
  private _selectedFamilias: Familias | null = null;
  ...
  mount(container: HTMLElement): void {
    ...
    pageTitle.textContent = 'Familias';
    pageTitle.style.margin = '0';

    const btnNew = document.createElement('button');
    btnNew.textContent = '+ Nuevo Familias';
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._familias = filterExcluded(await familiasService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private _preSelectFKValues(form: HTMLElement, data: Familias): void {
  ...
  private _refreshGrid(): void {
    this._gridie.setBody(
      toFamiliasGridRows(this._familias, {
  ...
  private _openCreate(): void {
    this._selectedFamilias = null;
    this._modalCreate.setBody(this._buildForm(null, this._modalCreate));
    this._modalCreate.open();
  }

  private _openEdit(item: Familias): void {
    this._selectedFamilias = item;
  ...
  private _openDelete(item: Familias): void {
    this._selectedFamilias = item;
  ...
  private _buildForm(initialData: Familias | null, modal: ModalXInstance): HTMLElement {
    ...
    const form = FormX({
      ...
      onSubmit: async (result) => {
        ...
        try {
          const data: FamiliasCreateDTO = {
            sucursal_id: result.body['sucursal_id'] as number,
            nombre: result.body['nombre'] as string,
            color: result.body['color'] as string,
            icono: result.body['icono'] as string,
            orden_visual: result.body['orden_visual'] as number,
            destino_impresion: result.body['destino_impresion'] as string,
          };
          ...
          if (isEdit) {
            ...
            toastx.success('Familias creado correctamente');
          }
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedFamilias || this._saving) return;
      this._saving = true;
...
export function Familias(container: HTMLElement): FamiliasFeature {
  const feature = new FamiliasFeature();
