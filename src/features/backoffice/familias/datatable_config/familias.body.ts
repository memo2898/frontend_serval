import type { Familias, FamiliasGridRow } from '../familias.types';

export interface FamiliasHandlers {
  onEdit: (familias: Familias) => void;
  onDelete: (familias: Familias) => void;
}
...
export const toFamiliasGridRow = (familias: Familias, handlers: FamiliasHandlers): FamiliasGridRow => ({
  id: familias.id,
  sucursal: familias.sucursal_?.nombre ?? String(familias.sucursal_id ?? ''),
  ...
  actualizado_por: familias.actualizado_por,
  actions: [
    {
      content: `
        ${actionButtonStyles}
      ...
      `,
      event: 'click',
      funct: () => handlers.onEdit(familias),
    },
    {
      content: `
      ...
      `,
      event: 'click',
      funct: () => handlers.onDelete(familias),
    },
  ],
});

export const toFamiliasGridRows = (familias: Familias[], handlers: FamiliasHandlers): FamiliasGridRow[] =>
  familias.map((familias) => toFamiliasGridRow(familias, handlers));
