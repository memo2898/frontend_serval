// ============================================
// Feature: ConfiguracionSucursals — CRUD (Vanilla TS)
// ============================================
...
import * as configuracionSucursalService from './configuracionsucursal.service';
import { configuracionSucursalHeaders } from './datatable_config/configuracionsucursal.headers';
import { toConfiguracionSucursalGridRows } from './datatable_config/configuracionsucursal.body';
import { getConfiguracionSucursalFields } from './form_config/configuracionsucursal.fields';
import { buildDeleteBody } from './form_config/configuracionsucursal.delete';
import type { ConfiguracionSucursal, ConfiguracionSucursalCreateDTO } from './configuracionsucursal.types';
...
export class ConfiguracionSucursalFeature {
  // State
  private _configuracionSucursals: ConfiguracionSucursal[] = [];
  private _saving = false;
  private _selectedConfiguracionSucursal: ConfiguracionSucursal | null = null;
  ...
  mount(container: HTMLElement): void {
    ...
    pageTitle.textContent = 'ConfiguracionSucursals';
    pageTitle.style.margin = '0';

    const btnNew = document.createElement('button');
    btnNew.textContent = '+ Nuevo ConfiguracionSucursal';
    ...
    this._gridie = new Gridie({
      id: 'configuracionsucursal-table',
      identityField: 'id',
      headers: configuracionSucursalHeaders,
      body: [],
      enableSort: true,
      enableFilter: true,
      language: 'es',
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._configuracionSucursals = filterExcluded(await configuracionSucursalService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private _preSelectFKValues(form: HTMLElement, data: ConfiguracionSucursal): void {
  ...
  private _refreshGrid(): void {
    this._gridie.setBody(
      toConfiguracionSucursalGridRows(this._configuracionSucursals, {
  ...
  private _openCreate(): void {
    this._selectedConfiguracionSucursal = null;
    this._modalCreate.setBody(this._buildForm(null, this._modalCreate));
    this._modalCreate.open();
  }

  private _openEdit(item: ConfiguracionSucursal): void {
    this._selectedConfiguracionSucursal = item;
  ...
  private _openDelete(item: ConfiguracionSucursal): void {
    this._selectedConfiguracionSucursal = item;
  ...
  private _buildForm(initialData: ConfiguracionSucursal | null, modal: ModalXInstance): HTMLElement {
    ...
    const form = FormX({
      ...
      onSubmit: async (result) => {
        ...
        try {
          const data: ConfiguracionSucursalCreateDTO = {
            sucursal_id: result.body['sucursal_id'] as number,
            tiene_mesas: result.body['tiene_mesas'] as boolean,
            tiene_delivery: result.body['tiene_delivery'] as boolean,
            tiene_barra: result.body['tiene_barra'] as boolean,
            impuesto_defecto_id: result.body['impuesto_defecto_id'] as number,
            tarifa_defecto_id: result.body['tarifa_defecto_id'] as number,
            moneda: result.body['moneda'] as string,
            formato_fecha: result.body['formato_fecha'] as string,
            zona_horaria: result.body['zona_horaria'] as string,
            permite_venta_sin_stock: result.body['permite_venta_sin_stock'] as boolean,
            requiere_mesa_para_orden: result.body['requiere_mesa_para_orden'] as boolean,
            imprime_automatico_al_cerrar: result.body['imprime_automatico_al_cerrar'] as boolean,
          };
          ...
          if (isEdit) {
            ...
            toastx.success('ConfiguracionSucursal creado correctamente');
          }
      ...
      children: [
        ...getConfiguracionSucursalFields(initialData, {
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedConfiguracionSucursal || this._saving) return;
      this._saving = true;
...
export function ConfiguracionSucursal(container: HTMLElement): ConfiguracionSucursalFeature {
  const feature = new ConfiguracionSucursalFeature();
