...
export function buildDeleteBody(item: ConfiguracionSucursal): HTMLElement {
  const wrapper = document.createElement('div');

  const question = document.createElement('p');
  question.style.marginBottom = '16px';
  question.textContent = '¿Estás seguro que deseas eliminar el siguiente configuracionsucursal?';
  ...
  secondary.textContent = String(item.tiene_mesas ?? '');
