// ============================================
// Feature: Alergenos — CRUD (Vanilla TS)
// ============================================
...
import * as alergenosService from './alergenos.service';
import { alergenosHeaders } from './datatable_config/alergenos.headers';
import { toAlergenosGridRows } from './datatable_config/alergenos.body';
import { getAlergenosFields } from './form_config/alergenos.fields';
import { buildDeleteBody } from './form_config/alergenos.delete';
import type { Alergenos, AlergenosCreateDTO } from './alergenos.types';
...
export class AlergenosFeature {
  // State
  private _alergenos: Alergenos[] = [];
  private _saving = false;
  private _selectedAlergenos: Alergenos | null = null;
  ...
  mount(container: HTMLElement): void {
    ...
    pageTitle.textContent = 'Alergenos';
    pageTitle.style.margin = '0';

    const btnNew = document.createElement('button');
    btnNew.textContent = '+ Nuevo Alergenos';
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._alergenos = filterExcluded(await alergenosService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private _refreshGrid(): void {
    this._gridie.setBody(
      toAlergenosGridRows(this._alergenos, {
  ...
  private _buildForm(initialData: Alergenos | null, modal: ModalXInstance): HTMLElement {
    ...
    const form = FormX({
      ...
      onSubmit: async (result) => {
        ...
        try {
          ...
          if (isEdit) {
            ...
            toastx.success('Alergenos creado correctamente');
          }
      ...
      children: [
        ...getAlergenosFields(initialData),
        errorMsg,
        actions,
      ],
    });
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedAlergenos || this._saving) return;
      this._saving = true;
...
export function Alergenos(container: HTMLElement): AlergenosFeature {
  const feature = new AlergenosFeature();
