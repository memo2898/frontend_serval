...
export function buildDeleteBody(item: Alergenos): HTMLElement {
  const wrapper = document.createElement('div');

  const question = document.createElement('p');
  question.style.marginBottom = '16px';
  question.textContent = '¿Estás seguro que deseas eliminar el siguiente alergenos?';
  ...
  secondary.textContent = String(item.icono ?? '');
