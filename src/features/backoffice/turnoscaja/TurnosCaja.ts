// ============================================
// Feature: TurnosCajas — CRUD (Vanilla TS)
// ============================================
...
import * as turnosCajaService from './turnoscaja.service';
import { turnosCajaHeaders } from './datatable_config/turnoscaja.headers';
import { toTurnosCajaGridRows } from './datatable_config/turnoscaja.body';
import { getTurnosCajaFields } from './form_config/turnoscaja.fields';
import { buildDeleteBody } from './form_config/turnoscaja.delete';
import type { TurnosCaja, TurnosCajaCreateDTO } from './turnoscaja.types';
...
export class TurnosCajaFeature {
  // State
  private _turnosCajas: TurnosCaja[] = [];
  private _saving = false;
  private _selectedTurnosCaja: TurnosCaja | null = null;
  ...
  mount(container: HTMLElement): void {
    ...
    pageTitle.textContent = 'TurnosCajas';
    pageTitle.style.margin = '0';

    const btnNew = document.createElement('button');
    btnNew.textContent = '+ Nuevo TurnosCaja';
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._turnosCajas = filterExcluded(await turnosCajaService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private async _fetchRelatedOptions(): Promise<void> {
    try {
      const [terminalesRaw, usuariosRaw] = await Promise.all([
  ...
  private _preSelectFKValues(form: HTMLElement, data: TurnosCaja): void {
  ...
  private _refreshGrid(): void {
    this._gridie.setBody(
      toTurnosCajaGridRows(this._turnosCajas, {
  ...
  private _openCreate(): void {
    this._selectedTurnosCaja = null;
    this._modalCreate.setBody(this._buildForm(null, this._modalCreate));
    this._modalCreate.open();
  }

  private _openEdit(item: TurnosCaja): void {
    this._selectedTurnosCaja = item;
  ...
  private _openDelete(item: TurnosCaja): void {
    this._selectedTurnosCaja = item;
  ...
  private _buildForm(initialData: TurnosCaja | null, modal: ModalXInstance): HTMLElement {
    ...
    const form = FormX({
      ...
      onSubmit: async (result) => {
        ...
        try {
          const data: TurnosCajaCreateDTO = {
            ...
            monto_apertura: result.body['monto_apertura'] as number,
            monto_cierre_declarado: result.body['monto_cierre_declarado'] as number,
            monto_cierre_real: result.body['monto_cierre_real'] as number,
          };
          ...
          if (isEdit) {
            ...
            toastx.success('TurnosCaja creado correctamente');
          }
      ...
      children: [
        ...getTurnosCajaFields(initialData, {
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedTurnosCaja || this._saving) return;
      this._saving = true;
...
export function TurnosCaja(container: HTMLElement): TurnosCajaFeature {
  const feature = new TurnosCajaFeature();
