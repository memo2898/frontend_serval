...
export function buildDeleteBody(item: Reservaciones): HTMLElement {
  const wrapper = document.createElement('div');

  const question = document.createElement('p');
  question.style.marginBottom = '16px';
  question.textContent = '¿Estás seguro que deseas eliminar el siguiente reservaciones?';
  ...
  secondary.textContent = String(item.mesa_id ?? '');
