...
export function buildDeleteBody(item: OrdenPagos): HTMLElement {
  const wrapper = document.createElement('div');

  const question = document.createElement('p');
  question.style.marginBottom = '16px';
  question.textContent = '¿Estás seguro que deseas eliminar el siguiente ordenpagos?';
  ...
  secondary.textContent = String(item.forma_pago_id ?? '');
