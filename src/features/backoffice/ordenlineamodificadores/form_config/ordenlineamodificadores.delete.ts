...
export function buildDeleteBody(item: OrdenLineaModificadores): HTMLElement {
  const wrapper = document.createElement('div');

  const question = document.createElement('p');
  question.style.marginBottom = '16px';
  question.textContent = '¿Estás seguro que deseas eliminar el siguiente ordenlineamodificadores?';

  const card = document.createElement('div');
  card.style.cssText = 'padding:16px;background:#fef2f2;border-radius:8px;border:1px solid #fecaca';

  const primary = document.createElement('strong');
  primary.textContent = String(item.orden_linea_id ?? item.id);

  card.appendChild(primary);

  const secondary = document.createElement('span');
  secondary.style.cssText = 'display:block;color:#6b7280;font-size:14px;margin-top:4px';
  secondary.textContent = String(item.modificador_id ?? '');
