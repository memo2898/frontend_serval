...
import { getAuthFields } from './form_config/auth.fields';
import { buildDeleteBody } from './form_config/auth.delete';
import type { Auth, AuthCreateDTO } from './auth.types';
...
export class AuthFeature {
  ...
  private _selectedAuth: Auth | null = null;
  ...
  mount(container: HTMLElement): void {
    ...
    pageTitle.textContent = 'Auths';
    pageTitle.style.margin = '0';

    const btnNew = document.createElement('button');
    btnNew.textContent = '+ Nuevo Auth';
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._auths = filterExcluded(await authService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private _buildForm(initialData: Auth | null, modal: ModalXInstance): HTMLElement {
    ...
    const form = FormX({
      ...
      onSubmit: async (result) => {
        ...
        try {
          const data: AuthCreateDTO = {
            username: result.body['username'] as string,
            password: result.body['password'] as string,
          };
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedAuth || this._saving) return;
      this._saving = true;
...
export function Auth(container: HTMLElement): AuthFeature {
