import type { Auth, AuthGridRow } from '../auth.types';

export interface AuthHandlers {
  onEdit: (auth: Auth) => void;
  onDelete: (auth: Auth) => void;
}
...
export const toAuthGridRow = (auth: Auth, handlers: AuthHandlers): AuthGridRow => ({
  id: auth.id,
  ...
  actions: [
    ...
    },
    {
      ...
      `,
      event: 'click',
      funct: () => handlers.onDelete(auth),
    },
  ],
});

export const toAuthGridRows = (auths: Auth[], handlers: AuthHandlers): AuthGridRow[] =>
  auths.map((auth) => toAuthGridRow(auth, handlers));
