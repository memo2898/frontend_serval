import type { Empresas, EmpresasGridRow } from '../empresas.types';

export interface EmpresasHandlers {
  onEdit: (empresas: Empresas) => void;
  onDelete: (empresas: Empresas) => void;
}
...
export const toEmpresasGridRow = (empresas: Empresas, handlers: EmpresasHandlers): EmpresasGridRow => ({
  id: empresas.id,
  ...
  actualizado_en: empresas.actualizado_en,
  actualizado_por: empresas.actualizado_por,
  actions: [
    {
      content: `
        ${actionButtonStyles}
      ...
      `,
      event: 'click',
      funct: () => handlers.onEdit(empresas),
    },
    {
      content: `
      ...
      `,
      event: 'click',
      funct: () => handlers.onDelete(empresas),
    },
  ],
});

export const toEmpresasGridRows = (empresas: Empresas[], handlers: EmpresasHandlers): EmpresasGridRow[] =>
  empresas.map((empresas) => toEmpresasGridRow(empresas, handlers));
