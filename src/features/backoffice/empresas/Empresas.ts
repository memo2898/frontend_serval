...
import type { Empresas, EmpresasCreateDTO } from './empresas.types';
import * as tipoDocumentosService from '@/features/backoffice/tipodocumentos/tipodocumentos.service';
import { uploadImage } from '@/features/backoffice/uploads/uploads.service';
...
export class EmpresasFeature {
  ...
  const uploaded = await uploadImage('empresas', logoFiles[0]);
  logoUrl = uploaded.url;
}

      // 2. Guardar empresa con la URL del logo
     const data: EmpresasCreateDTO = {
  ...
  estado:            '',
      ...
      if (isEdit) {
        await empresasService.update(initialData.id, data);
        toastx.success('Empresas actualizado correctamente');
      } else {
        await empresasService.create(data);
        toastx.success('Empresas creado correctamente');
      }
