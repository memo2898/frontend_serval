// ============================================
// Feature: Clientes — CRUD (Vanilla TS)
// ============================================
...
import { clientesHeaders } from './datatable_config/clientes.headers';
import { toClientesGridRows } from './datatable_config/clientes.body';
import { getClientesFields } from './form_config/clientes.fields';
import { buildDeleteBody } from './form_config/clientes.delete';
import type { Clientes, ClientesCreateDTO } from './clientes.types';
...
export class ClientesFeature {
  // State
  private _clientes: Clientes[] = [];
  private _saving = false;
  private _selectedClientes: Clientes | null = null;
  ...
  mount(container: HTMLElement): void {
    ...
    btnNew.textContent = '+ Nuevo Clientes';
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._clientes = filterExcluded(await clientesService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private async _fetchRelatedOptions(): Promise<void> {
    try {
      const [empresasRaw, tipoDocumentosRaw] = await Promise.all([
  ...
  private _preSelectFKValues(form: HTMLElement, data: Clientes): void {
  ...
  private _refreshGrid(): void {
    this._gridie.setBody(
      toClientesGridRows(this._clientes, {
  ...
  private _openCreate(): void {
    this._selectedClientes = null;
    this._modalCreate.setBody(this._buildForm(null, this._modalCreate));
    this._modalCreate.open();
  }

  private _openEdit(item: Clientes): void {
    this._selectedClientes = item;
  ...
  private _openDelete(item: Clientes): void {
    this._selectedClientes = item;
  ...
  private _buildForm(initialData: Clientes | null, modal: ModalXInstance): HTMLElement {
      ...
      onSubmit: async (result) => {
        ...
        try {
          ...
          if (isEdit) {
            ...
            toastx.success('Clientes creado correctamente');
          }
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedClientes || this._saving) return;
      this._saving = true;
...
export function Clientes(container: HTMLElement): ClientesFeature {
  const feature = new ClientesFeature();
