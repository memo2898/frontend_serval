...
export function buildDeleteBody(item: Paises): HTMLElement {
  const wrapper = document.createElement('div');

  const question = document.createElement('p');
  question.style.marginBottom = '16px';
  question.textContent = '¿Estás seguro que deseas eliminar el siguiente paises?';
  ...
  secondary.textContent = String(item.codigo_iso ?? '');
