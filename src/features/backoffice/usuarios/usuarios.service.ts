...
import type { Usuarios, UsuariosCreateDTO, UsuariosUpdateDTO, UsuariosFilters, PaginatedResponse } from './usuarios.types';

const BASE = `${SERVER_ROUTE}/api/usuarios`;
const IS_SOFT_DELETE = true;

export const getAll = async () => {
  return filterExcluded(await http.get<Usuarios[]>(BASE));
};

export const getById = async (id: number) => {
  return await http.get<Usuarios>(`${BASE}/${id}`);
};

export const create = async (data: UsuariosCreateDTO) => {
  return await http.post<Usuarios>(BASE, withCreateMeta(data));
};

export const update = async (id: number, data: UsuariosUpdateDTO) => {
  return await http.patch<Usuarios>(`${BASE}/${id}`, withUpdateMeta(data));
};
...
export const getPaginated = async (filters: UsuariosFilters = {}) => {
  const query = buildQuery(filters);
  const response = await http.get<PaginatedResponse<Usuarios>>(`${BASE}/paginated?${query}`);
  if (!filters.estado) {
