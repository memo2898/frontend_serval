// ============================================
// Feature: Descuentos — CRUD (Vanilla TS)
// ============================================
...
import { descuentosHeaders } from './datatable_config/descuentos.headers';
import { toDescuentosGridRows } from './datatable_config/descuentos.body';
import { getDescuentosFields } from './form_config/descuentos.fields';
import { buildDeleteBody } from './form_config/descuentos.delete';
import type { Descuentos, DescuentosCreateDTO } from './descuentos.types';
...
export class DescuentosFeature {
  // State
  private _descuentos: Descuentos[] = [];
  private _saving = false;
  private _selectedDescuentos: Descuentos | null = null;
  ...
  mount(container: HTMLElement): void {
    ...
    pageTitle.textContent = 'Descuentos';
    pageTitle.style.margin = '0';

    const btnNew = document.createElement('button');
    btnNew.textContent = '+ Nuevo Descuentos';
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._descuentos = filterExcluded(await descuentosService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private _preSelectFKValues(form: HTMLElement, data: Descuentos): void {
  ...
  private _refreshGrid(): void {
    this._gridie.setBody(
      toDescuentosGridRows(this._descuentos, {
  ...
  private _openCreate(): void {
    this._selectedDescuentos = null;
    this._modalCreate.setBody(this._buildForm(null, this._modalCreate));
    this._modalCreate.open();
  }

  private _openEdit(item: Descuentos): void {
    this._selectedDescuentos = item;
  ...
  private _openDelete(item: Descuentos): void {
    this._selectedDescuentos = item;
  ...
  private _buildForm(initialData: Descuentos | null, modal: ModalXInstance): HTMLElement {
    ...
    const form = FormX({
      ...
      onSubmit: async (result) => {
        ...
        try {
          const data: DescuentosCreateDTO = {
            sucursal_id: result.body['sucursal_id'] as number,
            nombre: result.body['nombre'] as string,
            tipo: result.body['tipo'] as string,
            valor: result.body['valor'] as number,
            aplica_a: result.body['aplica_a'] as string,
            requiere_autorizacion: result.body['requiere_autorizacion'] as boolean,
            fecha_inicio: result.body['fecha_inicio'] as string,
            fecha_fin: result.body['fecha_fin'] as string,
          };
          ...
          if (isEdit) {
            ...
            toastx.success('Descuentos creado correctamente');
          }
      ...
      children: [
        ...getDescuentosFields(initialData, {
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedDescuentos || this._saving) return;
      this._saving = true;
...
export function Descuentos(container: HTMLElement): DescuentosFeature {
  const feature = new DescuentosFeature();
