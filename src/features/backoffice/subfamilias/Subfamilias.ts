// ============================================
// Feature: Subfamilias — CRUD (Vanilla TS)
// ============================================
...
import * as subfamiliasService from './subfamilias.service';
import { subfamiliasHeaders } from './datatable_config/subfamilias.headers';
import { toSubfamiliasGridRows } from './datatable_config/subfamilias.body';
import { getSubfamiliasFields } from './form_config/subfamilias.fields';
import { buildDeleteBody } from './form_config/subfamilias.delete';
import type { Subfamilias, SubfamiliasCreateDTO } from './subfamilias.types';
...
export class SubfamiliasFeature {
  // State
  private _subfamilias: Subfamilias[] = [];
  private _saving = false;
  private _selectedSubfamilias: Subfamilias | null = null;
  ...
  mount(container: HTMLElement): void {
    ...
    pageTitle.textContent = 'Subfamilias';
    pageTitle.style.margin = '0';

    const btnNew = document.createElement('button');
    btnNew.textContent = '+ Nuevo Subfamilias';
    ...
    this._gridie = new Gridie({
      id: 'subfamilias-table',
      identityField: 'id',
      headers: subfamiliasHeaders,
      body: [],
      enableSort: true,
      enableFilter: true,
      language: 'es',
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._subfamilias = filterExcluded(await subfamiliasService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private async _fetchRelatedOptions(): Promise<void> {
    try {
      const [familiasRaw] = await Promise.all([
  ...
  private _preSelectFKValues(form: HTMLElement, data: Subfamilias): void {
  ...
  private _refreshGrid(): void {
    this._gridie.setBody(
      toSubfamiliasGridRows(this._subfamilias, {
  ...
  private _openCreate(): void {
    this._selectedSubfamilias = null;
    this._modalCreate.setBody(this._buildForm(null, this._modalCreate));
    this._modalCreate.open();
  }

  private _openEdit(item: Subfamilias): void {
    this._selectedSubfamilias = item;
  ...
  private _openDelete(item: Subfamilias): void {
    this._selectedSubfamilias = item;
  ...
  private _buildForm(initialData: Subfamilias | null, modal: ModalXInstance): HTMLElement {
    ...
    const form = FormX({
      ...
      onSubmit: async (result) => {
        ...
        try {
          ...
          if (isEdit) {
            ...
            toastx.success('Subfamilias creado correctamente');
          }
      ...
      children: [
        ...getSubfamiliasFields(initialData, {
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedSubfamilias || this._saving) return;
      this._saving = true;
...
export function Subfamilias(container: HTMLElement): SubfamiliasFeature {
  const feature = new SubfamiliasFeature();
