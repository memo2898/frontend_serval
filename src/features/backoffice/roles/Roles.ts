...
import { getRolesFields } from './form_config/roles.fields';
import { buildDeleteBody } from './form_config/roles.delete';
import type { Roles, RolesCreateDTO } from './roles.types';
...
export class RolesFeature {
  // State
  private _roles: Roles[] = [];
  private _saving = false;
  private _selectedRoles: Roles | null = null;
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._roles = filterExcluded(await rolesService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private _openDelete(item: Roles): void {
    this._selectedRoles = item;
  ...
  private _buildForm(initialData: Roles | null, modal: ModalXInstance): HTMLElement {
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedRoles || this._saving) return;
      this._saving = true;
...
export function Roles(container: HTMLElement): RolesFeature {
  const feature = new RolesFeature();
