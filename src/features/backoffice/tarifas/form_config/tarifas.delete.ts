import type { Tarifas } from '../tarifas.types';

export function buildDeleteBody(item: Tarifas): HTMLElement {
  const wrapper = document.createElement('div');

  const question = document.createElement('p');
  question.style.marginBottom = '16px';
  question.textContent = '¿Estás seguro que deseas eliminar el siguiente tarifas?';
