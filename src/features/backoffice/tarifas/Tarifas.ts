// ============================================
// Feature: Tarifas — CRUD (Vanilla TS)
// ============================================
...
import * as tarifasService from './tarifas.service';
import { tarifasHeaders } from './datatable_config/tarifas.headers';
import { toTarifasGridRows } from './datatable_config/tarifas.body';
import { getTarifasFields } from './form_config/tarifas.fields';
import { buildDeleteBody } from './form_config/tarifas.delete';
import type { Tarifas, TarifasCreateDTO } from './tarifas.types';
...
export class TarifasFeature {
  // State
  private _tarifas: Tarifas[] = [];
  private _saving = false;
  private _selectedTarifas: Tarifas | null = null;
  ...
  mount(container: HTMLElement): void {
    ...
    pageTitle.textContent = 'Tarifas';
    pageTitle.style.margin = '0';

    const btnNew = document.createElement('button');
    btnNew.textContent = '+ Nuevo Tarifas';
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._tarifas = filterExcluded(await tarifasService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private _preSelectFKValues(form: HTMLElement, data: Tarifas): void {
  ...
  private _refreshGrid(): void {
    this._gridie.setBody(
      toTarifasGridRows(this._tarifas, {
  ...
  private _openCreate(): void {
    this._selectedTarifas = null;
    this._modalCreate.setBody(this._buildForm(null, this._modalCreate));
    this._modalCreate.open();
  }

  private _openEdit(item: Tarifas): void {
    this._selectedTarifas = item;
  ...
  private _openDelete(item: Tarifas): void {
    this._selectedTarifas = item;
  ...
  private _buildForm(initialData: Tarifas | null, modal: ModalXInstance): HTMLElement {
      ...
      onSubmit: async (result) => {
        ...
        try {
          ...
          if (isEdit) {
            ...
            toastx.success('Tarifas creado correctamente');
          }
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedTarifas || this._saving) return;
      this._saving = true;
...
export function Tarifas(container: HTMLElement): TarifasFeature {
  const feature = new TarifasFeature();
