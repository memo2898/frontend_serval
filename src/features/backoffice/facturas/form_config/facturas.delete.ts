...
export function buildDeleteBody(item: Facturas): HTMLElement {
  const wrapper = document.createElement('div');

  const question = document.createElement('p');
  question.style.marginBottom = '16px';
  question.textContent = '¿Estás seguro que deseas eliminar el siguiente facturas?';
  ...
  secondary.textContent = String(item.cliente_id ?? '');
