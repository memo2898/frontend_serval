// ============================================
// Feature: Facturas — CRUD (Vanilla TS)
// ============================================
...
import { facturasHeaders } from './datatable_config/facturas.headers';
import { toFacturasGridRows } from './datatable_config/facturas.body';
import { getFacturasFields } from './form_config/facturas.fields';
import { buildDeleteBody } from './form_config/facturas.delete';
import type { Facturas, FacturasCreateDTO } from './facturas.types';
...
export class FacturasFeature {
  // State
  private _facturas: Facturas[] = [];
  private _saving = false;
  private _selectedFacturas: Facturas | null = null;
  ...
  mount(container: HTMLElement): void {
    ...
    btnNew.textContent = '+ Nuevo Facturas';
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._facturas = filterExcluded(await facturasService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private async _fetchRelatedOptions(): Promise<void> {
    try {
      const [ordenesRaw, clientesRaw] = await Promise.all([
  ...
  private _preSelectFKValues(form: HTMLElement, data: Facturas): void {
  ...
  private _refreshGrid(): void {
    this._gridie.setBody(
      toFacturasGridRows(this._facturas, {
  ...
  private _openCreate(): void {
    this._selectedFacturas = null;
    this._modalCreate.setBody(this._buildForm(null, this._modalCreate));
    this._modalCreate.open();
  }

  private _openEdit(item: Facturas): void {
    this._selectedFacturas = item;
  ...
  private _openDelete(item: Facturas): void {
    this._selectedFacturas = item;
  ...
  private _buildForm(initialData: Facturas | null, modal: ModalXInstance): HTMLElement {
      ...
      onSubmit: async (result) => {
        ...
        try {
          ...
          if (isEdit) {
            ...
            toastx.success('Facturas creado correctamente');
          }
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedFacturas || this._saving) return;
      this._saving = true;
...
export function Facturas(container: HTMLElement): FacturasFeature {
  const feature = new FacturasFeature();
