// ============================================
// Feature: KdsOrdenes — CRUD (Vanilla TS)
// ============================================
...
import * as kdsOrdenesService from './kdsordenes.service';
import { kdsOrdenesHeaders } from './datatable_config/kdsordenes.headers';
import { toKdsOrdenesGridRows } from './datatable_config/kdsordenes.body';
import { getKdsOrdenesFields } from './form_config/kdsordenes.fields';
import { buildDeleteBody } from './form_config/kdsordenes.delete';
import type { KdsOrdenes, KdsOrdenesCreateDTO } from './kdsordenes.types';
...
export class KdsOrdenesFeature {
  // State
  private _kdsOrdenes: KdsOrdenes[] = [];
  private _saving = false;
  private _selectedKdsOrdenes: KdsOrdenes | null = null;
  ...
  mount(container: HTMLElement): void {
    ...
    pageTitle.textContent = 'KdsOrdenes';
    pageTitle.style.margin = '0';

    const btnNew = document.createElement('button');
    btnNew.textContent = '+ Nuevo KdsOrdenes';
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._kdsOrdenes = filterExcluded(await kdsOrdenesService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private async _fetchRelatedOptions(): Promise<void> {
    try {
      const [ordenLineasRaw] = await Promise.all([
  ...
  private _preSelectFKValues(form: HTMLElement, data: KdsOrdenes): void {
  ...
  private _refreshGrid(): void {
    this._gridie.setBody(
      toKdsOrdenesGridRows(this._kdsOrdenes, {
  ...
  private _openCreate(): void {
    this._selectedKdsOrdenes = null;
    this._modalCreate.setBody(this._buildForm(null, this._modalCreate));
    this._modalCreate.open();
  }

  private _openEdit(item: KdsOrdenes): void {
    this._selectedKdsOrdenes = item;
  ...
  private _openDelete(item: KdsOrdenes): void {
    this._selectedKdsOrdenes = item;
  ...
  private _buildForm(initialData: KdsOrdenes | null, modal: ModalXInstance): HTMLElement {
    ...
    const form = FormX({
      ...
      onSubmit: async (result) => {
        ...
        try {
          const data: KdsOrdenesCreateDTO = {
            orden_linea_id: result.body['orden_linea_id'] as number,
            destino_id: result.body['destino_id'] as number,
            tiempo_recibido: result.body['tiempo_recibido'] as string,
            tiempo_preparado: result.body['tiempo_preparado'] as string,
          };
          ...
          if (isEdit) {
            ...
            toastx.success('KdsOrdenes creado correctamente');
          }
      ...
      children: [
        ...getKdsOrdenesFields(initialData, {
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedKdsOrdenes || this._saving) return;
      this._saving = true;
...
export function KdsOrdenes(container: HTMLElement): KdsOrdenesFeature {
  const feature = new KdsOrdenesFeature();
