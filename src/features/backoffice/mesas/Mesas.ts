...
import { mesasHeaders } from './datatable_config/mesas.headers';
import { toMesasGridRows } from './datatable_config/mesas.body';
import { getMesasFields } from './form_config/mesas.fields';
import { buildDeleteBody } from './form_config/mesas.delete';
import type { Mesas, MesasCreateDTO } from './mesas.types';
...
export class MesasFeature {
  // State
  private _mesas: Mesas[] = [];
  private _saving = false;
  private _selectedMesas: Mesas | null = null;
  private _zonasOptions: Array<{ value: number; label: string }> = [];
  ...
  mount(container: HTMLElement): void {
    ...
    btnNew.textContent = '+ Nuevo Mesas';
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._mesas = filterExcluded(await mesasService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private async _fetchRelatedOptions(): Promise<void> {
    try {
      const [zonasRaw] = await Promise.all([
        zonasService.getAll(),
      ]);
  ...
  private _preSelectFKValues(form: HTMLElement, data: Mesas): void {
  ...
  private _openDelete(item: Mesas): void {
    this._selectedMesas = item;
  ...
  private _buildForm(initialData: Mesas | null, modal: ModalXInstance): HTMLElement {
    ...
    const form = FormX({
      ...
      onSubmit: async (result) => {
        ...
        try {
          ...
          if (isEdit) {
            ...
            toastx.success('Mesas creado correctamente');
          }
      ...
      children: [
        ...getMesasFields(initialData, {
          zonasOptions: this._zonasOptions,
        }),
        errorMsg,
        actions,
      ],
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedMesas || this._saving) return;
      this._saving = true;
...
export function Mesas(container: HTMLElement): MesasFeature {
  const feature = new MesasFeature();
