import type { Mesas, MesasGridRow } from '../mesas.types';

export interface MesasHandlers {
  onEdit: (mesas: Mesas) => void;
  onDelete: (mesas: Mesas) => void;
}
...
export const toMesasGridRow = (mesas: Mesas, handlers: MesasHandlers): MesasGridRow => ({
  ...
  actions: [
    ...
    },
    {
      ...
      `,
      event: 'click',
      funct: () => handlers.onDelete(mesas),
    },
  ],
});

export const toMesasGridRows = (mesas: Mesas[], handlers: MesasHandlers): MesasGridRow[] =>
  mesas.map((mesas) => toMesasGridRow(mesas, handlers));
