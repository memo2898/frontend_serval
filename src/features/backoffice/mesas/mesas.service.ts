...
import type { Mesas, MesasCreateDTO, MesasUpdateDTO, MesasFilters, PaginatedResponse } from './mesas.types';

const BASE = `${SERVER_ROUTE}/api/mesas`;
const IS_SOFT_DELETE = true;

export const getAll = async () => {
  return filterExcluded(await http.get<Mesas[]>(BASE));
};

export const getById = async (id: number) => {
  return await http.get<Mesas>(`${BASE}/${id}`);
};
...
export const update = async (id: number, data: MesasUpdateDTO) => {
  return await http.patch<Mesas>(`${BASE}/${id}`, withUpdateMeta(data));
};
...
export const getPaginated = async (filters: MesasFilters = {}) => {
  const query = buildQuery(filters);
  const response = await http.get<PaginatedResponse<Mesas>>(`${BASE}/paginated?${query}`);
  if (!filters.estado) {
