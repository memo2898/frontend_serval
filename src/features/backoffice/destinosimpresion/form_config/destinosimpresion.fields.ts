...
export function getDestinosImpresionFields(
  initialData: DestinosImpresion | null,
  options: {
  ...
  return [
    ...
    InputX({
      name: 'ip_impresora',
      label: 'Ip Impresora',
      placeholder: 'Ingrese ip impresora',
      type: 'text',
      defaultValue: initialData?.ip_impresora != null ? String(initialData.ip_impresora) : '',
      rules: {
        validations: [
          { type: 'required', message: 'Ip Impresora es obligatorio' },
          { type: 'maxLength', value: 255 },
        ],
      },
    }),
