// ============================================
// Feature: MovimientosCajas — CRUD (Vanilla TS)
// ============================================
...
import * as movimientosCajaService from './movimientoscaja.service';
import { movimientosCajaHeaders } from './datatable_config/movimientoscaja.headers';
import { toMovimientosCajaGridRows } from './datatable_config/movimientoscaja.body';
import { getMovimientosCajaFields } from './form_config/movimientoscaja.fields';
import { buildDeleteBody } from './form_config/movimientoscaja.delete';
import type { MovimientosCaja, MovimientosCajaCreateDTO } from './movimientoscaja.types';
...
export class MovimientosCajaFeature {
  // State
  private _movimientosCajas: MovimientosCaja[] = [];
  private _saving = false;
  private _selectedMovimientosCaja: MovimientosCaja | null = null;
  ...
  mount(container: HTMLElement): void {
    ...
    pageTitle.textContent = 'MovimientosCajas';
    pageTitle.style.margin = '0';

    const btnNew = document.createElement('button');
    btnNew.textContent = '+ Nuevo MovimientosCaja';
    ...
    this._gridie = new Gridie({
      id: 'movimientoscaja-table',
      identityField: 'id',
      headers: movimientosCajaHeaders,
      body: [],
      enableSort: true,
      enableFilter: true,
      language: 'es',
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._movimientosCajas = filterExcluded(await movimientosCajaService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private _refreshGrid(): void {
    this._gridie.setBody(
      toMovimientosCajaGridRows(this._movimientosCajas, {
  ...
  private _buildForm(initialData: MovimientosCaja | null, modal: ModalXInstance): HTMLElement {
    ...
    const form = FormX({
      ...
      onSubmit: async (result) => {
        ...
        try {
          ...
          if (isEdit) {
            ...
            toastx.success('MovimientosCaja creado correctamente');
          }
      ...
      children: [
        ...getMovimientosCajaFields(initialData),
        errorMsg,
        actions,
      ],
    });
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedMovimientosCaja || this._saving) return;
      this._saving = true;
...
export function MovimientosCaja(container: HTMLElement): MovimientosCajaFeature {
  const feature = new MovimientosCajaFeature();
