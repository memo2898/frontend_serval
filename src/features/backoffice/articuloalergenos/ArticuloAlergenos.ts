// ============================================
// Feature: ArticuloAlergenos — CRUD (Vanilla TS)
// ============================================
...
import * as articuloAlergenosService from './articuloalergenos.service';
import { articuloAlergenosHeaders } from './datatable_config/articuloalergenos.headers';
import { toArticuloAlergenosGridRows } from './datatable_config/articuloalergenos.body';
import { getArticuloAlergenosFields } from './form_config/articuloalergenos.fields';
import { buildDeleteBody } from './form_config/articuloalergenos.delete';
import type { ArticuloAlergenos, ArticuloAlergenosCreateDTO } from './articuloalergenos.types';
import * as articulosService from '@/features/backoffice/articulos/articulos.service';
import * as alergenosService from '@/features/backoffice/alergenos/alergenos.service';
...
export class ArticuloAlergenosFeature {
  // State
  private _articuloAlergenos: ArticuloAlergenos[] = [];
  private _saving = false;
  private _selectedArticuloAlergenos: ArticuloAlergenos | null = null;
  private _articulosOptions: Array<{ value: number; label: string }> = [];
  private _alergenosOptions: Array<{ value: number; label: string }> = [];
  ...
  mount(container: HTMLElement): void {
    ...
    pageTitle.textContent = 'ArticuloAlergenos';
    pageTitle.style.margin = '0';

    const btnNew = document.createElement('button');
    btnNew.textContent = '+ Nuevo ArticuloAlergenos';
    ...
    this._gridie = new Gridie({
      id: 'articuloalergenos-table',
      identityField: 'id',
      headers: articuloAlergenosHeaders,
      body: [],
      enableSort: true,
      enableFilter: true,
      language: 'es',
  ...
  private async _fetch(): Promise<void> {
    ...
    try {
      this._articuloAlergenos = filterExcluded(await articuloAlergenosService.getAll());
      this._refreshGrid();
    } catch (err) {
  ...
  private async _fetchRelatedOptions(): Promise<void> {
    try {
      const [articulosRaw, alergenosRaw] = await Promise.all([
        articulosService.getAll(),
        alergenosService.getAll(),
      ]);
      this._articulosOptions = filterExcluded(articulosRaw).map((item: any) => ({
        value: item.id,
        label: item.familia_id ?? String(item.id),
      }));
      this._alergenosOptions = filterExcluded(alergenosRaw).map((item: any) => ({
        value: item.id,
  ...
  private _preSelectFKValues(form: HTMLElement, data: ArticuloAlergenos): void {
  ...
  private _refreshGrid(): void {
    this._gridie.setBody(
      toArticuloAlergenosGridRows(this._articuloAlergenos, {
  ...
  private _openCreate(): void {
    this._selectedArticuloAlergenos = null;
    this._modalCreate.setBody(this._buildForm(null, this._modalCreate));
    this._modalCreate.open();
  }

  private _openEdit(item: ArticuloAlergenos): void {
    this._selectedArticuloAlergenos = item;
  ...
  private _openDelete(item: ArticuloAlergenos): void {
    this._selectedArticuloAlergenos = item;
  ...
  private _buildForm(initialData: ArticuloAlergenos | null, modal: ModalXInstance): HTMLElement {
    ...
    const form = FormX({
      ...
      onSubmit: async (result) => {
        ...
        try {
          const data: ArticuloAlergenosCreateDTO = {
            articulo_id: result.body['articulo_id'] as number,
            alergeno_id: result.body['alergeno_id'] as number,
          };
          ...
          if (isEdit) {
            ...
            toastx.success('ArticuloAlergenos creado correctamente');
          }
      ...
      children: [
        ...getArticuloAlergenosFields(initialData, {
          articulosOptions: this._articulosOptions,
          alergenosOptions: this._alergenosOptions,
        }),
        errorMsg,
        actions,
      ],
  ...
  private _buildDeleteFooter(): HTMLElement[] {
    ...
    deleteBtn.addEventListener('click', async () => {
      if (!this._selectedArticuloAlergenos || this._saving) return;
      this._saving = true;
...
export function ArticuloAlergenos(container: HTMLElement): ArticuloAlergenosFeature {
  const feature = new ArticuloAlergenosFeature();
